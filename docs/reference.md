...
./masqr copilot                         # Copilot CLI (GitHub)
...
| `copilot`, `github-copilot` | `github-copilot` | `https://api.githubcopilot.com` | `COPILOT_API_URL` (full plaintext endpoint override — see below) | `authorization` | — |
...
**GitHub Copilot (`copilot`) is redirected via the `COPILOT_API_URL` env var.** The Copilot CLI (`@github/copilot`) normally resolves its API base from the signed-in user's `copilotUser.endpoints.api` (`api.githubcopilot.com` for individuals, `api.{business,enterprise}.githubcopilot.com` for orgs), but it honors **`COPILOT_API_URL`** as a full endpoint override that wins over everything else (recovered from the `Wa()` endpoint resolver in `app.js` v1.0.63, which returns `process.env.COPILOT_API_URL` first and logs `Using COPILOT_API_URL from environment`). An `http://` value makes the CLI's fetch/SDK clients speak **plaintext HTTP**, so `masqr copilot` exports `COPILOT_API_URL=http://<listener>` and takes the same ordinary plaintext reverse-proxy path as every other provider — no TLS intercept. Its agent loop is **multi-wire**: the conversation turn rides whichever API the active model speaks — the OpenAI **Responses** API for OpenAI models (`gpt-5-mini` POSTs `/responses`) and the Anthropic **Messages** API for Claude models (`claude-haiku-4.5` POSTs `/v1/messages`), both observed live (streaming, `X-Initiator: user`). `/chat/completions` carries only auxiliary traffic (session-title generation), `/agents/sessions/{id}/events` carries session telemetry, and the MCP endpoint (`/mcp`) rides the same host too — so a single upstream covers everything with no routes. The API key rides in `Authorization: Bearer`, so that header joins the redaction set. The interactive surface dispatches by request path: `/responses` and `/v1/messages` are the model-turn paths, so a blocked conversation turn comes back as a synthetic assistant turn in that wire's own shape — Copilot renders the block advice (rule, the `mask` offer, `/clear`, threshold tips) inline and supports the `mask` reply regardless of model; other endpoints fall back to the OpenAI 451 envelope.
...
| Variable | Effect |
|---|---|
| `ANTHROPIC_BASE_URL` / `GOOGLE_GEMINI_BASE_URL` / `OPENAI_BASE_URL` / `COPILOT_API_URL` | the provider profile picks the right one based on the child command name; override with `-e VAR` for unrecognised CLIs |
