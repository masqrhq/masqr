...
**GitHub Copilot (`copilot`) is redirected via the `COPILOT_API_URL` env var.** The Copilot CLI (`@github/copilot`) normally resolves its API base from the signed-in user's `copilotUser.endpoints.api` (`api.githubcopilot.com` for individuals, `api.{business,enterprise}.githubcopilot.com` for orgs), but it honors **`COPILOT_API_URL`** as a full endpoint override that wins over everything else (recovered from the `Wa()` endpoint resolver in `app.js` v1.0.63, which returns `process.env.COPILOT_API_URL` first and logs `Using COPILOT_API_URL from environment`). An `http://` value makes the CLI's fetch/SDK clients speak **plaintext HTTP**, so `masqr copilot` exports `COPILOT_API_URL=http://<listener>` and takes the same ordinary plaintext reverse-proxy path as every other provider — no TLS intercept. The CLI's default wire API is OpenAI chat completions (the native CAPI path POSTs `/chat/completions`; it can also speak `/responses` and `/v1/messages` under an explicit `COPILOT_PROVIDER_WIRE_API`), and the MCP endpoint (`/mcp`) rides the same host — so a single upstream covers chat, models, and MCP traffic with no routes. The API key rides in `Authorization: Bearer`, so that header joins the redaction set. A blocked chat turn comes back as a synthetic OpenAI-shaped assistant turn (so Copilot renders the block advice inline and supports the `mask` reply); non-chat endpoints fall back to the OpenAI 451 envelope.
