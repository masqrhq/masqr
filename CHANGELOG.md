...
### Changed
- **The forward-proxy MITM CA is now unique-per-install and persistent, generated once and checked at startup.** Previously masqr minted a brand-new in-memory CA on every run and wrote its certificate to `$PWD/masqr-ca.pem`, so each invocation produced a different root and littered the working directory. The source-of-truth CA (certificate **and** private key) now lives in a stable per-install location under the OS config dir — `<UserConfigDir>/masqr/ca/` (falling back to the cache dir), with the private key written `0600`. At startup masqr checks for an existing CA: if it is present, valid, and its stamped masqr version matches the running binary, it is **reused**; otherwise (fresh install, missing/corrupt store, or an upgrade to a new release — the masqr version is stamped alongside the CA) a fresh unique CA is **generated** and persisted. The chosen path is logged clearly (`generated new MITM CA …` vs `reusing persisted MITM CA …`). `MASQR_CA_FILE` is retained purely as an override for *where the certificate PEM is exported* for client trust; with no override the child is pointed straight at the persisted cert instead of a `$PWD` copy. If no config/cache dir is resolvable masqr degrades to an ephemeral in-memory CA, exactly as before.
...
### Added
- **Forward-proxy + MITM mode: masqr now inspects ALL of the wrapped child's HTTPS egress, not just the one reverse-proxied upstream.** Previously a wrapped agent (agy/antigravity, codex, …) still spoke *directly* to every other host it knew — `oauth2.googleapis.com`, `www.googleapis.com`, telemetry, auto-updaters — none of which passed through masqr. On every run masqr now ALSO acts as an HTTP forward proxy on the same `-a` listener: it answers `CONNECT host:port`, terminates TLS with an on-the-fly leaf certificate signed by a masqr-generated in-memory CA, runs the existing rule engine (plus OCR) over the decrypted request, BLOCKs/redacts on findings exactly like the reverse path, then re-encrypts and forwards upstream. Plain-HTTP (absolute-form) proxy requests are inspected the same way; the existing reverse-proxy path is unchanged. The child is wired up automatically — `HTTPS_PROXY`/`HTTP_PROXY` point at masqr, `SSL_CERT_FILE` (Go/OpenSSL) and `NODE_EXTRA_CA_CERTS` (Node/Electron) point at a trust bundle that includes the masqr CA, and `NO_PROXY` excludes loopback so the reverse-proxy `BASE_URL` traffic doesn't loop back. The CA certificate is written to the path in `MASQR_CA_FILE` (default `$PWD/masqr-ca.pem`) and logged at startup. Everything degrades gracefully: if the CA can't be generated or written, masqr logs once and keeps the reverse-proxy-only behaviour. Proven end-to-end by `scripts/fullproxy_check.sh` (an HTTPS request carrying a sample key to an arbitrary host is blocked with HTTP 451 and the `aws-access-key-id` rule recorded in the session log).
...
### Security
- **The forward proxy never blocks or strips the wrapped agent's own authentication credentials.** With the MITM forward proxy inspecting all egress, an agent's own OAuth token exchange (a `grant_type=refresh_token` POST whose body carries the `client_secret`/`refresh_token`) could be mistaken for a leaked secret and rejected with HTTP 451 in `--on-finding block` mode — silently breaking the agent's ability to authenticate. Requests to the agent's own credential endpoints (`accounts.google.com`, `oauth2.googleapis.com`, `www.googleapis.com`) are now forwarded **without inspection**, so those exchanges always pass through intact even in block mode. The `Authorization` and `Proxy-Authorization` headers are likewise forwarded untouched (`Proxy-Authorization`, a hop-by-hop header Go's reverse proxy would otherwise drop, is re-injected). Masking the `Authorization` value in masqr's own session log is unaffected; only leaked secrets in request *bodies* to other hosts are redacted/blocked.
